alert("Jogador vermelho: W & S para se mover \r\Jogador Azul: setas up e down para se mover.");
...
function sendIotRequest(path) {
  fetch('http:192.168.1.1:80/'+path, {
    method: 'GET',
    headers: {
...
function goal(pos) {
  ...
  if (Number(rscore.innerHTML) == 5) {
    
    alert("Player 1 wins");
    sendIotRequest("/player1");
  }
  else if (Number(lscore.innerHTML) == 5) {
    
    alert("Player 2 wins");
    sendIotRequest("/player2");
  ...
  sendIotRequest();
  if(Number(rscore.innerHTML) == 5 || Number(lscore.innerHTML) == 5){
    alert("Jogador vermelho: W e S para se mover \r\Jogador Azul: setas up e down para se mover.");
    rscore.innerHTML = 0;
    lscore.innerHTML = 0;
  }
}
